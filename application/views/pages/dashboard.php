...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
	...
	<div class="row">
		...
		<div class="col-3 mb-5 chart-wrapper">
			...
			<p>Target: <?= number_format($chart->target).' '.$chart->unit ?><br> Actual: <?= number_format($chart->actual).' '.$chart->unit ?></p>
			<div>
				<?php if ($this->session->permissions->create_form): ?>
				<nav class="nav d-inline-flex">
					<a class="nav-link py-0" data-toggle="Edit" title="Edit" href="<?= base_url('chart/'.$chart->chart_id.'/edit') ?>"><i data-feather="edit-2"></i></a>
					<a class="nav-link py-0 confirm-chart-delete" data-toggle="Delete" title="Delete" href="<?= base_url('chart/'.$chart->chart_id.'/delete') ?>"><i data-feather="trash"></i></a>
				</nav>
				<?php endif; ?>
			</div>
		</div>
	...
	<div class="row mt-3 mb-5">
		<div class="col">
			<div class="meter-widget mb-2">
				...
				<small><?= $storage['_dev_vda1']['Used'] ?>B of <?= $storage['_dev_vda1']['Size'] ?>B used</small>
			</div>
		</div>
		<div class="col">
			<div class="meter-widget">
				...
				<small><?= $storage['udev']['Used'] ?>B of <?= $storage['udev']['Size'] ?>B used</small>
			</div>		
		</div>
	</div>
<?php endif; ?>
